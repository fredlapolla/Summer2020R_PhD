---
title: "Intro to R for Data Analysis I"
author: "Fred LaPolla"
date: "July 6, 2020"
output:
...
This class has two portions: an introduction to R and R Studio, which are tools for data analysis, followed by an introduction to biostatistics and data analysis, which will build on the skills learned in the first half to enable you to draw insights from your data.
...
You will have weekly homework assignments to solidify lessons learned each week by working through problems in R and statistics. 

There will be a midterm project in R to find and load in data, clean and transform the data, and provide summary information about the data using R Markdown language. There will be regular assignments relating to this work to help keep everyone on track of where they should be in the process. 

A final exam will test understanding of statistical concepts. The final exam **will not** include R. 
...
>- Automate analysis and save it for later, which can save time for yourself and collaborators
...
A comprehensive reference for integrating Git with R: https://happygitwithr.com/ 
...
For demo purposes: https://github.com/fredwillie/R_PhDProg_NYU_2020
...
In addition to scripts and projects, R offers a file format called a Markdown file (.rmd). Your midterm project will be submitted as a markdown file. 

A markdown file combines human readable text with executable code in chunks. In this document, the code chunks have a gray background. This is a great way to present information by providing both written information and analysis together.

These slides are made with a markdown file. 
...
Two options are available: R Notebooks and R Markdown; one runs all code at once and the other chunk by chunk. This difference will not matter for this course, and my recommendation is to do R Markdown presentation as HTML slides. The PDF and Word options can require some fussing with on your computer's settings, so this is why I recommend for this class creating an HTML option. 
...
You can get more information on commands by going to Help > Cheatsheets > Markdown Cheatsheets

Some key formatting is 

command + option + i inserts a code chunk. This is excecutable code amid text. 
...
>- A more direct option is to just say 
>- data <- read.csv("data.csv")

>- assuming you have a csv on your computer calle data

>- What this does is sets a website, in this case the location of the raw crusio1 data on github, then tells "read.csv()" to use that URL as its dataframe location. 
...
install.packages("pastecs",repos = "http://cran.us.r-project.org")
library(pastecs)
round(stat.desc(crusio1$jump, norm = TRUE), 2)
```
...
- The name of the dataframe with a dollarsign followed by a variable: crusio1$jump
...
```{R}
crusio1[,86] <- sum(crusio1[18:22],na.rm=TRUE)/5
```
...
```{r echo=TRUE}
attackers <- filter(crusio1, attack_combine == 1)
head(attackers)
```
