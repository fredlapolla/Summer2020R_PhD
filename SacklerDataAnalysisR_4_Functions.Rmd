...
```{r}
crusio1[,87]<-NA
colnames(crusio1[87])<-"weightDichotomy"
for(i in 1:923){ifelse(crusio1$bw[i] > 30, crusio1$weightDichotomy[i] <- 1, crusio1$weightDichotomy[i] <- 0)}
crusio1$weightDichotomy<-factor(crusio1$weightDichotomy, levels = 0:1)
```
...
Make a filtered subset of straing to only contain BXD100 using tidyverse's Filter command.
...
>- Both apply and for loops provide options in R for iterating through data.

>- Ifelse offers a way to have something happen if some condition is true and is helpful for simplifying our data. This should always be done along theoretical lines relevant to current understandings of the underlying science (in other words, do not just dichotomize by median)
